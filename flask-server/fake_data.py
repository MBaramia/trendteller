...
fakePredicton = {
    ...
    for outer_key, inner_dict in fakeFetched.items()
}

combinedData = {
    outer_key: {**fakeData.get(outer_key, {}), **fakePredicton.get(outer_key, {})}
    for outer_key in set(fakeData) | set(fakePredicton)
}
...
for outer_key, inner_dict in fakePredicton.items():
    predictedDates[outer_key] = sorted(inner_dict.keys(), key=lambda date: datetime.strptime(date, "%Y-%m-%d %H:%M:%S") if ' ' in date else datetime.strptime(date, "%Y-%m-%d"), reverse=True)
