...
function StockChart({ company }) {
  ...
  const produceNormalChartData = () => {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      for (const key of keys) {
        dataArray.push({
          date: key,
          close: parseFloat(stockData[i][key].close),
        });
      }
      dataArray[0] = {
  ...
  const producePredictedChartData = () => {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      for (const key of predKeys) {
        dataArray.push({
          date: key,
          prediction: parseFloat(stockData[i][key].close),
        });
      }
  ...
  const getChartData = () => {
    ...
    normal = normal
      .slice(0, Math.min(normal.length, timeScaleToInfo[timeScale].count))
      .reverse();

    let predicted = [
      ...predictedChartData[timeScaleToInfo[timeScale].type],
    ].reverse();
      ...
      return (
        <div className="custom-tooltip">
          <p className="tooltip-title">
            {label + (data.isPrediction ? " (predicted)" : "")}
          </p>
